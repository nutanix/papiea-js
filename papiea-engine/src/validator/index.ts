...
function convertValidatorMessagesToPapieaMessages(
    ...
    errors: Error[], translated_errors: Error[]) {
    ...
    for (let i = 0;i < errors.length;i++) {
        ...
        translated_errors.push(new Error(message))
    }
}
...
export interface Validator {
    validate_uuid(provider_prefix: string, provider_version: string, kind: Kind, uuid: string): void
...
export class ValidatorImpl {
    ...
    public validate_uuid(provider_prefix: string, provider_version: string, kind: Kind, uuid: string) {
        const validation_pattern = kind.uuid_validation_pattern
        if (validation_pattern === undefined) {
            if (!uuid_validate(uuid)) {
                throw new PapieaException({ message: `Invalid entity UUID for kind: ${provider_prefix}/${provider_version}/${kind.name}. Make sure the entity has valid UUID.`, entity_info: { provider_prefix, provider_version, kind_name: kind.name, additional_info: { "entity_uuid": uuid }}})
            }
        } else {
            const regex = new RegExp(validation_pattern, 'g')
            if (!regex.test(uuid)) {
                throw new PapieaException({ message: `Entity UUID for kind: ${provider_prefix}/${provider_version}/${kind.name} does not match the pattern. Make sure the entity UUID matches the validation pattern.`, entity_info: { provider_prefix, provider_version, kind_name: kind.name, additional_info: { "entity_uuid": uuid }}})
            }
        }
    }
    ...
    public validate_metadata_extension(extension_structure: Data_Description, metadata: Metadata | undefined, allowExtraProps: boolean) {
        ...
        if (metadata.extension !== undefined && metadata.extension !== null && typeof metadata.extension !== "object") {
            throw new ValidationError({ message: `Failed to validate metadata extension for kind: ${metadata.provider_prefix}/${metadata.provider_version}/${metadata.kind}. Metadata extension should be an object.`, entity_info: { provider_prefix: metadata.provider_prefix, provider_version: metadata.provider_version, kind_name: metadata.kind, additional_info: { "entity_uuid": uuid.toString(), "metadata_extension": JSON.stringify(metadata.extension) }}})
        }
        if (metadata.extension === undefined || metadata.extension === null || isEmpty(metadata.extension)) {
            throw new ValidationError({ message: `Failed to validate metadata extension for kind: ${metadata.provider_prefix}/${metadata.provider_version}/${metadata.kind}. Make sure the metadata extension is defined.`, entity_info: { provider_prefix: metadata.provider_prefix, provider_version: metadata.provider_version, kind_name: metadata.kind, additional_info: { "entity_uuid": metadata.uuid }}})
        }
   ...
   /**
    ...
    public async validate_status(provider: Provider, entity_ref: Entity_Reference, status: Status) {
        if (status === undefined || isEmpty(status)) {
            throw new ValidationError({ message: `Failed to validate status for kind: ${provider.prefix}/${provider.version}/${entity_ref.kind}. Use null values instead of undefined to remove the status field.`, entity_info: { provider_prefix: provider.prefix, provider_version: provider.version, kind_name: entity_ref.kind, additional_info: { "entity_uuid": entity_ref.uuid }}})
        }
        const kind = provider.kinds.find((kind: Kind) => kind.name === entity_ref.kind);
        const allowExtraProps = provider.allowExtraProps;
        if (kind === undefined) {
            throw new PapieaException({ message: `Unable to find kind: ${provider.prefix}/${provider.version}/${entity_ref.kind} in provider. Make sure the name is correct & kind is registered.`, entity_info: { provider_prefix: provider.prefix, provider_version: provider.version, kind_name: entity_ref.kind, additional_info: { "entity_uuid": entity_ref.uuid }}});
        }
    ...
    public validate(
        ...
        if (modelIsEmpty(model)) {
            if (isEmpty(data)) {
                ...
                throw new ValidationError({
                    ...
                    entity_info: { provider_prefix: provider_prefix, provider_version: provider_version, kind_name: kind_name, additional_info: { "procedure_name": procedureName ?? '', "schema_name": schemaName, "received_input": JSON.stringify(data) }}
                })
            }
        }
        if (model !== undefined && model !== null) {
            ...
            if (!res.valid) {
                let translated_errors: Error[] = []
                convertValidatorMessagesToPapieaMessages(provider_prefix, provider_version, kind_name, res.errors, translated_errors)
                throw new ValidationError({
