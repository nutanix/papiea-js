    ...
    public async create(input: any, ctx: RequestContext): Promise<EntityCreationResult> {
        ...
        try {
            ...
            if (err instanceof ValidationError) {
                throw new OnActionError(`Entity returned by the custom constructor of kind ${this.provider.prefix}/${this.provider.version}/${this.kind.name} is not valid due to errors: ${(err as ValidationError).errors}`, { provider_prefix: this.provider.prefix, provider_version: this.provider.version, kind_name: this.kind.name, additional_info: { "entity_uuid": entity.metadata.uuid, "procedure_name": `__${this.kind.name}_create` }})
            } else {
                throw err
            }
        }
        ...
        if (!spec_status_equal && this.kind.intentful_behaviour === IntentfulBehaviour.SpecOnly) {
            throw new OnActionError(`Spec-only entity constructor returned of kind ${this.provider.prefix}/${this.provider.version}/${this.kind.name} spec not matching status`, { provider_prefix: this.provider.prefix, provider_version: this.provider.version, kind_name: this.kind.name, additional_info: { "entity_uuid": entity.metadata.uuid, "procedure_name": `__${this.kind.name}_create` }})
        }
        ...
        if (this.kind) {
            ...
            if (constructor !== undefined && constructor !== null) {
                if (this.user === undefined) {
                    throw new UnauthorizedError(`No user provided in the create entity request for kind ${this.provider.prefix}/${this.provider.version}/${this.kind.name}`, { provider_prefix: this.provider.prefix, provider_version: this.provider.version, kind_name: this.kind.name, additional_info: { "procedure_name": procedure_name }})
                }
                try {
                    const schemas: any = {}
                    ...
                    if (e instanceof ValidationError) {
                        ...
                        throw new OnActionError(e.response.data.message, { provider_prefix: this.provider.prefix, provider_version: this.provider.version, kind_name: this.kind.name, additional_info: { "procedure_name": procedure_name }})
                    }
                }
                if (
                    ...
                    throw new OnActionError(`Constructor return value is missing the spec or status field for entity of kind ${this.provider.prefix}/${this.provider.version}/${this.kind.name}`, { provider_prefix: this.provider.prefix, provider_version: this.provider.version, kind_name: this.kind.name, additional_info: { "procedure_name": procedure_name }})
                }
                return entity
            } else {
                // We should not reach this exception under normal condition because of pre checks while choosing strategy
                throw new PapieaException(`Entity creation was expecting a constructor but couldn't find it for kind ${this.provider.prefix}/${this.provider.version}/${this.kind.name}`, { provider_prefix: this.provider.prefix, provider_version: this.provider.version, kind_name: this.kind.name, additional_info: { "procedure_name": procedure_name }})
            }
        } else {
            throw new OnActionError(`Provider does not have any kind registered for provider ${this.provider.prefix}/${this.provider.version}`, { provider_prefix: this.provider.prefix, provider_version: this.provider.version, additional_info: { "procedure_name": procedure_name }})
        }
    }
}
