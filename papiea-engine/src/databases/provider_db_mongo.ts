...
export class Provider_DB_Mongo implements Provider_DB {
    ...
    async init(): Promise<void> {
        try {
            ...
            throw new PapieaException({ message: "Failed to setup the provider database.", cause: err });
        }
    }
    ...
    async save_provider(provider: Provider): Promise<void> {
        ...
        if (result.result.n !== 1) {
            throw new PapieaException({ message: `MongoDBError: Amount of updated provider entries should equal to 1, found ${ result.result.n } entries for provider: ${provider.prefix}/${provider.version}.`, entity_info: { provider_prefix: provider.prefix, provider_version: provider.version }})
        }
        ...
        try {
            ...
            if (err.code === 11000) {
                ...
                throw new PapieaException({ message: `Something went wrong in save provider for provider: ${provider.prefix}/${provider.version}.`, entity_info: { provider_prefix: provider.prefix, provider_version: provider.version }, cause: err})
            }
        }
    }
    ...
    async delete_provider(provider_prefix: string, version: Version): Promise<void> {
        ...
        if (result.ok !== 1) {
            throw new PapieaException({ message: `MongoDBError: Failed to remove provider: ${provider_prefix}/${version}.`, entity_info: { provider_prefix: provider_prefix, provider_version: version }});
        }
    ...
    async get_latest_provider_by_kind(kind_name: string): Promise<Provider> {
        ...
        if (providers.length === 0) {
            throw new PapieaException({ message: `MongoDBError: Provider with kind: ${ kind_name } not found.` });
        } else {
            return providers[0];
        }
    }
    ...
    async get_latest_provider(provider_prefix: string): Promise<Provider> {
        ...
        if (providers.length === 0) {
            throw new PapieaException({ message: `MongoDBError: Provider with prefix: ${ provider_prefix } not found.`, entity_info: { provider_prefix }});
        } else {
            return providers[0];
        }
    }
    ...
    find_kind(provider: Provider, kind_name: string): Kind {
        ...
        if (found_kind === undefined) {
            throw new PapieaException({ message: `MongoDBError: Kind not found in provider: ${provider.prefix}/${provider.version}/${kind_name}.`, entity_info: { provider_prefix: provider.prefix, provider_version: provider.version, kind_name: kind_name }});
        }
        return found_kind;
    }
}
